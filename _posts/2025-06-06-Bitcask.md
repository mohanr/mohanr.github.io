...
let%expect_test _=  (* Add 4 children to Node type 4*)
  let n = new_node4 in
  let new_node =
            let rec loop_while new_node idx=
            if idx < 4 then(
		            let newer_node = add_child (Bytes.make 1 (Char.chr idx)) new_node in
                    loop_while newer_node (idx + 1)
            )else new_node
            in
            loop_while n 0
    in
